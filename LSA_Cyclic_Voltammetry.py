...
def Open_LSACV_File():
    ...
    if Potenzialarrays[1] < Potenzialarrays[0]:
        DesiReactxx = 0
    #=========================================================
    #out of for-loop for LSA_Cyclic Voltammetry
    #=========================================================
...
def Get_LSACV_Data():
    ...
    def NextLSACV():
        ...
        def quit():
            Fenster.destroy()
        quit()
    Accept = Button(Fenster, text="Accept",command=AcceptLSACV)
    Accept.grid(row=8, column=0) 
    Next = Button(Fenster, text="Next",command=NextLSACV)
...
def LSA_Superpos_Shower():
    ...
    if FIT == 1:
        ...
        NormLengthxxx    = (1.0/len(Storage_x))
        NormArrayPot     = np.arange(0,1,NormLengthPot)
        NormArrayxxx     = np.arange(0,1,NormLengthxxx)
        Fitinterpolation = InterpolatedUnivariateSpline(NormArrayxxx,yyy_Add,k=3)
...
def LSA_Semi_Inf_Planar():
    LSA_Model_Zeroer()
    global model_a
    model_a = 1
    global FIT 
    FIT = 0
    LSA_Eingabe_CV_Simulator()
       
def LSA_Finit_Planar():
    LSA_Model_Zeroer()
    global model_b
    model_b = 1
    global FIT 
...
def LSA_Semi_Inf_Planar_FITTER():
    LSA_Model_Zeroer()
    global model_a
    model_a = 1
    global FIT 
...
def LSA_Finit_Planar_Trans_FITTER():
    LSA_Model_Zeroer()
    global model_b1
    model_b1 = 1
    global FIT 
...
def LSA_Semi_Inf_Zyl_Ext_FITTER():
    LSA_Model_Zeroer()
    global model_c
    model_c = 1
    global FIT 
...
def LSA_Semi_Inf_Sphere_Ext_FITTER():
    LSA_Model_Zeroer()
    global model_f
    model_f = 1
    global FIT 
...
def LSA_Finit_Sphere_Int_FITTER():
    LSA_Model_Zeroer()
    global model_g
    model_g = 1
    global FIT 
...
def LSA_Statistical_Finit_Planar():
    LSA_Model_Zeroer()
    global model_b
    global Statistical
    Statistical  = 1
...
def LSA_Statistical_Finit_Zyl_Ext():
    LSA_Model_Zeroer()
    global model_d
    model_d = 1
    global Statistical
...
def LSA_Statistical_Finit_Zyl_Int():
    LSA_Model_Zeroer()
    global model_e
    global Statistical
    Statistical  = 1
...
def LSA_Statistical_Finit_Sphere_Int():
    LSA_Model_Zeroer()
    global model_g
    global Statistical
    Statistical  = 1
...
def LSA_Statistical_Finit_Planar_FITTER():
    LSA_Model_Zeroer()
    global model_b
    global Statistical
    Statistical  = 1
...
def LSA_Statistical_Finit_Zyl_Ext_FITTER():
    LSA_Model_Zeroer()
    global model_d
    global Statistical
    Statistical  = 1
...
def LSA_Statistical_Finit_Zyl_Int_FITTER():
    LSA_Model_Zeroer()
    global model_e
    global Statistical
    Statistical  = 1
...
def LSA_Statistical_Finit_Sphere_Int_FITTER():
    LSA_Model_Zeroer()
    global model_g
    global Statistical
    Statistical  = 1
...
def LSA_Eingabe_CV_Simulator():
    ...
    if FIT == 0:
        PerDurat_Label = Label(Fenster,text="Period duration [s]*",bg = '#80ffbf')
        PerDurat_Label.place(x = 300, y = 85)
        PerDurat_Eingabe = Entry(Fenster)
        PerDurat_Eingabe.place(x = 425, y = 85)
    ...
    var17 = IntVar()
    Checkbutton(Fenster, text="Modify number of t-increments per period", bg = '#EFEFEF', variable=var17).place(x = 25, y = 315)
...
#========================================================================
        ...
        if FIT == 0:
            PeriodDuration = (float(PerDurat_Eingabe.get()))
...
#========================================================================
    ...
    button=Button(Fenster,text="Accept Parameters",bg = '#EFEFEF', command=LSA_AcceptParams).place(x = 570, y = 470, width = 260, height = 55)
    button=Button(Fenster,text="Next",bg = '#EFEFEF', command=LSA_CV_Type_Chooser).place(x = 570, y =535, width = 260, height = 55)
    


# In[ ]:

def LSA_CV_Type_Chooser():
    if model_a == 1:
        LSA_Start_End_Definer()
        if Statistical  == 0:
            LSA_model_a_ConvolutionInverter()
        if Statistical  == 1:
            LSA_DistFunc_Calcer()
    if model_b == 1:
        LSA_Start_End_Definer()
        if Statistical  == 0:
            LSA_model_b_ConvolutionInverter()
        if Statistical  == 1:
            LSA_DistFunc_Calcer()
    if model_b1 == 1:
        LSA_Start_End_Definer()
        if Statistical  == 0:
            LSA_model_b1_ConvolutionInverter()
        if Statistical  == 1:
            LSA_DistFunc_Calcer() 
    if model_c == 1:
        LSA_Start_End_Definer()
        if Statistical  == 0:
            LSA_model_c_ConvolutionInverter()
        if Statistical  == 1:
            LSA_DistFunc_Calcer()
    if model_d == 1:
        LSA_Start_End_Definer()
        if Statistical  == 0:
            LSA_model_d_ConvolutionInverter()
        if Statistical  == 1:
            LSA_DistFunc_Calcer()
    if model_e == 1:
        LSA_Start_End_Definer()
        if Statistical  == 0:
            LSA_model_e_ConvolutionInverter()
        if Statistical  == 1:
            LSA_DistFunc_Calcer()
    if model_f == 1:
        LSA_Start_End_Definer()
        if Statistical  == 0:
            LSA_model_f_ConvolutionInverter()
        if Statistical  == 1:
            LSA_DistFunc_Calcer()
    if model_g == 1:
        LSA_Start_End_Definer()
        if Statistical  == 0:
            LSA_model_g_ConvolutionInverter()
        if Statistical  == 1:
            LSA_DistFunc_Calcer()
...
def LSA_DistFunc_User():
    ...
    if INTERRUPT == 0:
        ...
        if FIT == 1:
            ...
            b.plot(xxx[1:-1:],CurrCalc[1:-1:],color='r', marker ='.', linestyle = '-')   
...
def LSA_model_a_ConvolutionInverter():
...
def LSA_model_b_ConvolutionInverter():
...
def LSA_model_b1_ConvolutionInverter():
    #also in statistical case distance will be passed as distance
    if distance < 0.00001 or distance > 10.0 or D_f <0.000001 or D_b <0.000001:
        Parameters_do_not_fit_warner()
...
def LSA_model_c_ConvolutionInverter():
    global a
    a = r
...
def LSA_model_d_ConvolutionInverter():
    global a
    a = r
...
def LSA_model_e_ConvolutionInverter():
    global a
    if Statistical == 0:
        a = r
...
def LSA_model_f_ConvolutionInverter():
    global a
    a = r
...
def LSA_model_g_ConvolutionInverter():
    global a
    if Statistical == 0:
        a = r
...
def LSA_CV_Calculator():
    ...
    global Xi_i
    if FIT == 0:
        PerDur = PeriodDuration
        if Cathsweeper == 0:
            E_switch        = SWT
            E_i             = SSS
            E_f             = EEE
            Xi_i            = n*F*E_i/(R*T)   #initial Xi
            Xi_switch       = n*F*E_switch/(R*T)  #final Xi
            Xi_f            = n*F*E_f/(R*T)
            if E_f < E_i:
                Overspan_Warner()
        if Cathsweeper == 1:
            ...
            if E_f > E_i:
                Overspan_Warner()
        t_Array_cont        = np.linspace(0,PerDur,1000)
        if Delta_t_Modder   == 1:
            t_Array_cont    = np.linspace(0,PerDur,int(ModDelt_t))
        t_Stop              = ( PerDur*(1 - (1/np.pi)*np.arcsin((Xi_f-Xi_i)/(Xi_switch-Xi_i))) )
        Stopindex           = (np.abs((t_Array_cont[int(len(t_Array_cont)*0.5)::] - t_Stop))).argmin() + int(len(t_Array_cont)*0.5)
        t_Array_cont        = t_Array_cont[0:Stopindex]
        Xi_Array        = Xi_i + (Xi_switch-Xi_i)*np.sin((np.pi/(PerDur))*t_Array_cont)     #not 2*pi in sine, because halt period wanted
    ...
    if FIT == 1:
        if DesiReactxx == 1:
            PeriodDuration  = np.asscalar(2.0*Time[(np.abs(Potenzialarrays - np.max(Potenzialarrays))).argmin()])
        if DesiReactxx == 0:
            PeriodDuration  = np.asscalar(2.0*Time[(np.abs(Potenzialarrays - np.min(Potenzialarrays))).argmin()])
        E_switch        = EEE
        E_i             = SSS
        E_f             = AsymmetrischesEnde
        Xi_i            = n*F*E_i/(R*T)   #initial Xi
        Xi_switch       = n*F*E_switch/(R*T)  #final Xi
        Xi_f            = n*F*E_f/(R*T)
        if E_f < E_i:
            Overspan_Warner()
        t_Array_cont        = np.linspace(0,PeriodDuration,1000)
        if Delta_t_Modder   == 1:
            t_Array_cont    = np.linspace(0,PeriodDuration,int(ModDelt_t))
        t_Stop              = ( PeriodDuration*(1 - (1/np.pi)*np.arcsin((Xi_f-Xi_i)/(Xi_switch-Xi_i))) )
        Stopindex           = (np.abs((t_Array_cont[int(len(t_Array_cont)*0.5)::] - t_Stop))).argmin() + int(len(t_Array_cont)*0.5)
        t_Array_cont        = t_Array_cont[0:Stopindex]
        
        Xi_Array        = Xi_i + (Xi_switch-Xi_i)*np.sin((np.pi/PeriodDuration)*t_Array_cont)     #not 2*pi in sine, because halt period wanted
        Exp_Array       = np.exp(-Xi_Array)
        Preced_Array    = np.exp(-p*t_Array_cont)
        Follow_Array    = np.exp(-f*t_Array_cont)
        Kinetik_Array   = D_f**0.5/(kzero*Exp_Array**(-alpha))
...
def Simulated_LSACV_as_txt_saver():
    root = Toplevel() 
    ...
    with open(root.filename,"w") as fi:
        ...
        if FIT == 0:
            fi.write("Period Duration [V/s]")
            fi.write("\t")
            fi.write(str(PeriodDuration))
            fi.write("\n")

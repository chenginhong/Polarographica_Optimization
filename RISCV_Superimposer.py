...
def RISCV_SIM_Superimposer():
    def RISCV_Runner():
        ...
        if choose == 2:
            RIS_Finit_Planar()
        if choose == 3:
            RIS_Finit_Transm_Planar()
        if choose == 4:
            RIS_Semi_Inf_Zyl_Ext()
        if choose == 5:
            RIS_Finit_Zyl_Ext()
        if choose == 6:
            RIS_Finit_Zyl_Int()
        if choose == 7:
            RIS_Semi_Inf_Sphere_Ext()
        if choose == 8:
            RIS_Finit_Sphere_Int()
        if choose == 9:
            RIS_Statistical_Finit_Planar()
        if choose == 10:
            RIS_Statistical_Finit_Zyl_Ext()
        if choose == 11:
            RIS_Statistical_Finit_Zyl_Int()
        if choose == 12:
            RIS_Statistical_Finit_Sphere_Int()
    Fenster = Toplevel()                                                         
    Fenster.title("Superposition mode") 
    Fenster.geometry("650x400")
    colorbgr = Label(Fenster, text= "", bg = '#80ffbf')
    colorbgr.place(x = 0, y = 0, width =7000, height = 2000)  
    CVType_Label  = Label(Fenster,text="Superimpose up to 50 RISCVs by choosing the CV-Type number 1 to 12", bg = '#80ffbf', font = ('Arial',12,'bold'))
    ...
    button0 =Button (Fenster,text="run",bg = '#EFEFEF', command=RISCV_Runner).place(x = 200, y = 85, width = 90, height = 50)
    var1 = IntVar()
    ...
    def RISCV_E_vs_I_Displayer():
        if RS_Mode == 1 and Add_Mode == 1:
            RISCV_E_vs_I_Double_Mode_Caller()
        if RS_Mode == 0 and Add_Mode == 0:
            RISCV_No_Mode_Warner()
        if RS_Mode == 1 and Add_Mode == 0:
            RISCV_E_vs_I_RS_Mode_Caller()
        if Add_Mode == 1 and RS_Mode == 0:
            RISCV_E_vs_I_Add_Mode_Caller()
    def RISCV_t_vs_I_Displayer():
        if RS_Mode == 1 and Add_Mode == 1:
            RISCV_t_vs_I_Double_Mode_Caller()
        if RS_Mode == 0 and Add_Mode == 0:
            RISCV_No_Mode_Warner()
        if RS_Mode == 1 and Add_Mode == 0:
            RISCV_t_vs_I_RS_Mode_Caller()
        if Add_Mode == 1 and RS_Mode == 0:
            RISCV_t_vs_I_Add_Mode_Caller()
    def RISCV_E_vs_I_Mode_Decider():
        global RS_Mode
        global Add_Mode
        Add_Mode  = var1.get()
        RS_Mode   = var2.get()
        RISCV_E_vs_I_Displayer()
    def RISCV_t_vs_I_Mode_Decider():
        global RS_Mode
        global Add_Mode
        Add_Mode  = var1.get()
        RS_Mode   = var2.get()
        RISCV_t_vs_I_Displayer()
    button1=Button(Fenster,text="Begin/Clear",bg = '#EFEFEF',        command=RISCV_Initializer    ).place(x = 25, y =300 , width = 100, height = 85)
    button2=Button(Fenster,text="Append latest",bg = '#EFEFEF',      command=RISCV_Append_Latest  ).place(x = 150, y =300 , width = 225, height = 35)
    button3=Button(Fenster,text="Remove latest",bg = '#EFEFEF',      command=RISCV_Remove_Latest  ).place(x = 150, y =350 , width = 225, height = 35)
    button4=Button(Fenster,text="Show E vs. I",bg = '#EFEFEF', command=RISCV_E_vs_I_Mode_Decider).place(x = 400, y =300 , width = 105, height = 35)
    button5=Button(Fenster,text="Show t vs. I",bg = '#EFEFEF', command=RISCV_t_vs_I_Mode_Decider).place(x = 520, y =300 , width = 105, height = 35)
    button6=Button(Fenster,text="Save Superposition",bg = '#EFEFEF', command=RISCV_Superpos_Saver ).place(x = 400, y =350 , width = 225, height = 35)
